from businesses.models import Table
from businesses.schemas import TableSchema
...
def read_tables(db: Session, skip: int = 0, limit: int = 100, capacity: int = 0):
    return db.query(Table).filter(Table.capacity == capacity).offset(skip).limit(limit).all()
