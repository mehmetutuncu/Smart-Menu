from businesses import schemas, crud
...
@router.get('/tables', response_model=List[schemas.TableSchema])
...
@router.post('/tables/', response_model=schemas.TableSchema, status_code=status.HTTP_201_CREATED)
async def create_table(table: schemas.TableSchema, db: Session = Depends(get_db)):
    return crud.create_table(db=db, table=table)


@router.patch(path='/tables/{table_id}', response_model=schemas.TableSchema, status_code=status.HTTP_200_OK)
